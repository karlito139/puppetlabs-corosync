...
Puppet::Type.type(:cs_rsc_defaults).provide(:crm, :parent => Puppet::Provider::Corosync) do
  ...
  def self.instances
    ...
    if ! doc.root.elements['configuration/rsc_defaults/meta_attributes'].nil?
      doc.root.elements['configuration/rsc_defaults/meta_attributes'].each_element do |e|
        items = e.attributes
  ...
  def flush
    unless @property_hash.empty?
      ...
      ENV['CIB_shadow'] = nil
      crm('configure', 'rsc_defaults', '$id="rsc-options-ressource"', "#{@property_hash[:name]}=#{@property_hash[:value]}")
    end
  end
end
